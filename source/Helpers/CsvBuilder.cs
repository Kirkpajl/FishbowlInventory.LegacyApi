...
namespace FishbowlInventory.Helpers
{
    ...
    {
        public string[] Build(params T[] items)
        {
        ...
        protected string[] GetRowValues(T item)
        {
            // Declare a collection of Property names and the desired sort orders
        ...
        protected string BuildRow(string[] values) => string.Join(',', values.Select(v => EscapeValue(v ?? string.Empty)));

        protected string EscapeValue(string value) => $"\"{value.Replace("\"", "\"\"")}\"";
    }
